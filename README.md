...
- 当前 NVIDIA/CUDA 基线正在做最终一致性与复现检查；完成后冻结稳定 tag，再进入 AMD/DirectML 与眨眼、EAR、PERCLOS 增量开发。
...
`nvidia-cuda` 是当前唯一维护分支，也是 GitHub default；它保存已经完成正式全量分析的 NVIDIA/CUDA 路线。AMD 路线使用 `amd-DirectML`，从经过最终检查并冻结的 NVIDIA 基线节点创建。
