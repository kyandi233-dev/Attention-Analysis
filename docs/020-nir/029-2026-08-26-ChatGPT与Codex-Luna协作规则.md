...
3. `030-NIR-cohort当前本地执行任务.md` + `031-NIR-cohort最近一次本地执行结果.md`：**当前一次本地交接**。030 是 ChatGPT 写给 Luna 的当前任务；031 是 Luna 执行后写给 ChatGPT 的最新结果。
...
若本地执行暴露出代码 bug、数据结构不一致或新的科学问题，Luna 应停止扩展设计，只在 031 报告现象、最小复现、相关路径/字段和错误信息，由 ChatGPT决定如何修改。
...
用户在 ChatGPT 说“继续”
        ↓
ChatGPT 规划/写代码/更新 028，并覆盖 030 当前任务
        ↓
用户对 Luna 说“读取 030 并执行”
        ↓
Luna 本地运行，只更新 031 最新结果
        ↓
用户回 ChatGPT 说“Luna 执行完了”
        ↓
ChatGPT 直接读取 031、代码和必要文件并继续
```

用户给 Luna 的固定一句话：

```text
读取 Attention-Analysis 仓库 `docs/020-nir/030-NIR-cohort当前本地执行任务.md`，严格按 029 的职责分工执行。不要自行修改分析计划或大规模写代码；完成后只更新 `031-NIR-cohort最近一次本地执行结果.md`。
```
