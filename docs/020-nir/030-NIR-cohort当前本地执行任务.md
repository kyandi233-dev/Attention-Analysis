...
> 这是 ChatGPT → Codex Luna 的**唯一当前执行入口**。每次只保留当前任务；旧任务不在这里累积。Luna 不需要翻 GitHub Issue 历史评论。
...
5. 更新 `031-NIR-cohort最近一次本地执行结果.md` 并 push。
...
如果 Git 同步、测试或正式运行失败，不修改代码；把最小错误写入 031 后停止。
...
```text
docs/020-nir/031-NIR-cohort最近一次本地执行结果.md
```

写入简短结果后 commit + push 到 `analysis/nir-behavior-v2`。除 031 外，不修改仓库其他文件。
