...
后续重新核对目标 NVIDIA 工作站硬件后，确认实际 GPU 为 **NVIDIA GeForce RTX 5070**。本工作记录前文中的 RTX 5060 是当时的阶段性误记，按工作记录不追溯改写的原则保留原文；从本更正条目起，当前操作文档、性能实测与正式补跑均以 **RTX 5070** 为准。

该更正只影响硬件型号记录，不改变本次 full-class 方法、`J:/Data` 数据根、RITnet `640×400 / FP32 / b16`、主指标、QC 规则或输出 schema。
