...
- **NIR**：正式全量分析已完成；当前进一步进入 cohort（多人样本）质量控制、标准化、行为/Probe 和多模态融合分析。
- **Behavior**：当前正式实现已经按 FocusWave v3.1.3 的 B1/B2 两-block 结构建立。
...
因此，核心编号不是全仓库单纯按创建时间连续递增，而是“**十位表示模块，个位表示该模块中的核心阅读顺序**”。

日期型历史资料继续使用 `MM-DD-序号-...` 或其他明确日期命名，因为日期本身就是 provenance（来源/过程追踪）。历史资料一旦不再承担核心入口职责，不应继续占用其他模块的核心编号。
