...
当前 E/F 盘数据根约定统一为：`E:/正式实验`、`E:/Data`、`F:/Data`。当前 BB reader 会在这些根目录中自动寻找 `sub-XXX_` 或 `sub-XXX` 被试目录；正式 BB 数据只纳入编号 ≥31 且同时具有 Block1/Block2 行为文件的完整被试。

旧 BBB 的配置、程序和结果均与当前 BB 分离：配置为 `sart_bbb_v3_0.yaml`，脚本为 `scripts/sart_bbb_v3_0_analysis.py`，实现包为 `src/attention_pipeline/behavior_bbb_v3_0/`。历史报告和图保存在 `docs/030-behavior/history/BBB-v3.0/`。
