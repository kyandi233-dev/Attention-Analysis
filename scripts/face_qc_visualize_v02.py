...
def _draw_full_mesh(frame: np.ndarray, row) -> None:
    ...
    for indices in [FACE_OVAL, LIPS_OUTER, NOSE_BRIDGE]:
        ...
        if len(pts) >= 2:
            ...
            cv2.polylines(frame, [poly], indices is not NOSE_BRIDGE, (190, 190, 190), 1, cv2.LINE_AA)
...
def _annotate_full(frame: np.ndarray, faces, eye_row):
    ...
    return v01._annotate(base, faces, eye_row)
