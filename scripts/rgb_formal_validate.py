...
IDENTITY_COLUMNS = {"subject", "video_frame_position", "capture_frame_idx", "unix_ms", "phase"}
...
def _read_parquet_projection(
    ...
    *,
    ...
    try:
        ...
        return table, int(parquet.metadata.num_rows), available
    except Exception as exc:
...
def validate_subject(config_path: str, subject: str) -> dict[str, Any]:
    ...
    _read_json(paths["face_prepare_manifest"], issues)
    ...
    if face_manifest and face_manifest.get("execution_backend") != "pytorch_cuda":
        issues.append(
            f"face_raw: NVIDIA formal manifest must report execution_backend=pytorch_cuda, got {face_manifest.get('execution_backend')!r}"
        )
    if face_manifest and not str(face_manifest.get("device", "")).lower().startswith("cuda"):
        issues.append(f"face_raw: CUDA device evidence missing/invalid: {face_manifest.get('device')!r}")
    ...
    face_raw, face_rows, face_cols = _read_parquet_projection(
        paths["face_raw"], wanted_columns=projection, issues=issues
    )
    motion_raw, motion_rows, motion_cols = _read_parquet_projection(
        paths["motion_raw"], wanted_columns=set(IDENTITY_COLUMNS), issues=issues
    )
    pose_raw, pose_rows, pose_cols = _read_parquet_projection(
    ...
    _check_identity(face_raw, available_columns=face_cols, label="face_raw", subject=subject, issues=issues)
    _check_identity(motion_raw, available_columns=motion_cols, label="motion_raw", subject=subject, issues=issues)
    _check_identity(pose_raw, available_columns=pose_cols, label="pose_landmarks", subject=subject, issues=issues)
    ...
    if not face_frames.empty and not face_raw.empty:
        if "benchmark_index" not in face_frames.columns or "benchmark_index" not in face_cols:
            ...
            if len(expected_ids) != len(output_ids):
                issues.append(f"face: planned/output frame mismatch {len(expected_ids)} != {len(output_ids)}")
            if missing_ids:
    ...
    if motion_manifest:
        ...
        if isinstance(first_pos, int) and isinstance(last_pos, int) and last_pos >= first_pos:
            ...
            if motion_rows != expected_motion_rows:
                issues.append(f"motion: expected/output row mismatch {expected_motion_rows} != {motion_rows}")
    ...
    if pose_manifest:
        ...
        if isinstance(sampled, int) and "video_frame_position" in pose_raw.columns:
            unique_pose_frames = int(pose_raw["video_frame_position"].nunique())
            if unique_pose_frames != sampled:
                issues.append(f"pose: sampled/output frame mismatch {sampled} != {unique_pose_frames}")
    ...
    result: dict[str, Any] = {
        ...
        "extraction_definition": "raw-first: Motion raw + Pose landmarks raw + Py-Feat native CUDA Face raw",
        "nvidia_face_backend_required": "pytorch_cuda",
        ...
        "files": {key: _file_record(path, row_counts.get(key)) for key, path in paths.items()},
        ...
        "notes": [
            ...
            "The validator reads only identity/index columns from wide parquet files.",
            "NVIDIA Face completion additionally requires explicit native PyTorch/CUDA backend evidence; CPU fallback is not accepted.",
        ],
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate one completed NVIDIA formal RGB raw extraction")
    ...
    if not result["extraction_complete"]:
        raise SystemExit(2)
    print(f"[rgb-nvidia:formal-validate] complete {args.subject}")
