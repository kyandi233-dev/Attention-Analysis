"""HISTORICAL executable: FocusWave v3.0 BBB SART behavior analysis.

This script intentionally preserves the 2026-08-16 three-block (BBB) analysis
for sub-011..030. It is not the current v3.1.3 BB analysis.

Current formal behavior:
    python scripts/sart_formal_analysis.py --stage all

Historical BBB rerun:
    python scripts/sart_bbb_v3_0_analysis.py --stage all
    python scripts/sart_bbb_v3_0_analysis.py --config configs/sart_bbb_v3_0.yaml --stage stats
"""
...
from attention_pipeline.metadata import run_metadata
from attention_pipeline.behavior_bbb_v3_0 import extract as fex
from attention_pipeline.behavior_bbb_v3_0 import metrics as fmet
from attention_pipeline.behavior_bbb_v3_0 import stats as fstat
...
def stage_extract(config) -> dict:
    ...
    _, _, behavior, manifests = _out_paths(config)
    ...
    manifest.update({
        "stage": "extract",
        "historical_pipeline": "FocusWave v3.0 BBB",
...
def stage_metrics(config, trials: pd.DataFrame) -> dict:
    ...
    _, _, behavior, _ = _out_paths(config)
...
def stage_stats(config, trials: pd.DataFrame) -> dict:
    ...
    main_df, _ = fstat.main_effects(config, blocks_p)
    ...
    for key, value in pa.items():
        if isinstance(value, pd.DataFrame):
            value.to_csv(behavior / f"051-probe_assoc_{key}.csv", index=False, encoding="utf-8-sig")
    ...
    (manifests / "051-correlation.json").write_text(
        json.dumps({k: v for k, v in corr.items() if k not in ("corr_matrix", "cross_block")},
                   ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    return {"main_rows": len(main_all), "drift": len(drift), "prenogo": len(prenogo)}


def main() -> None:
    ap = argparse.ArgumentParser(description="HISTORICAL FocusWave v3.0 BBB SART behavior analysis")
    ap.add_argument("--config", default="configs/sart_bbb_v3_0.yaml")
    ...
    for stage in stages:
        if stage == "extract":
            result[stage] = stage_extract(config)
            ...
            from attention_pipeline.behavior_bbb_v3_0 import figures as ffig
            if trials is None:
                trials = fex.load_cohort(config, config.section("subjects")["include"])
            result[stage] = ffig.generate_all(config, trials)
        elif stage == "report":
            from attention_pipeline.behavior_bbb_v3_0 import report as frep
            if trials is None:
                trials = fex.load_cohort(config, config.section("subjects")["include"])
