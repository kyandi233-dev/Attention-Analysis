...
def test_frame_axis_sync_ignores_rgb_baseline_when_phase_is_available():
    nir = pd.DataFrame({"session_id": ["s1"] * 4, "unix_ms": [1000, 1100, 1200, 1300]})
    events = pd.DataFrame({
        "session_id": ["s1", "s1"],
        "start_unix_ms": [90, 1090],
        "end_unix_ms": [110, 1210],
    })
    rgb = pd.DataFrame({
        "unix_ms": [100, 1010, 1110, 1210, 1310],
        "phase": ["baseline", "block1", "block1", "block1", "block1"],
    })
    row = audit_rgb_nir_sync_with_frames(nir, events, rgb).iloc[0]
    assert row["rgb_frame_time_min_ms"] == pytest.approx(1010)
    assert row["rgb_minus_nir_start_ms"] == pytest.approx(10)
    assert row["frame_nearest_residual_abs_median_ms"] == pytest.approx(10)
    assert row["blink_event_n"] == 1
    assert row["nearest_residual_abs_median_ms"] == pytest.approx(10)
