...
def _normalize_spec(spec: dict[str, Any]) -> tuple[dict[str, list[str]], list[str]]:
...
def build_analysis_sets(
    ...
    The missing-aware set still requires real/readable/aligned source and valid explicit
    QC. It only tolerates a missing feature value that remains eligible for a training-fold
    missing-data strategy. Structural modality absence is never admitted.
    """
    ...
    status_required = set(required_identity + [
        "modality", "feature", "feature_computable", "eligible_for_missing_strategy", "missing_kind"
    ])
    ...
    for analysis_set_id, raw_spec in comparison_specs.items():
        required, models = _normalize_spec(raw_spec)
        required_pairs = [(m, f) for m, features in required.items() for f in features]
        required_modalities = sorted(required)
        current_rows: list[dict[str, Any]] = []

        for identity_row in formal_identity[required_identity].itertuples(index=False, name=None):
            identity_values = dict(zip(required_identity, identity_row))
            chosen: list[dict[str, Any]] = []
            for modality, feature in required_pairs:
                lookup = tuple(identity_row) + (modality, feature)
                if lookup in status_index.index:
                    ...
                    chosen.append({
                        ...
                        "eligible_for_missing_strategy": bool(row["eligible_for_missing_strategy"]),
            ...
            complete = bool(selected["feature_computable"].all())
            missing_aware = bool(selected["eligible_for_missing_strategy"].all())
            ...
            record.update(
                {
                    ...
                    "complete_exclusion_reason": _aggregate_reasons(selected, "feature_computable"),
                    "missing_aware_exclusion_reason": _aggregate_reasons(selected, "eligible_for_missing_strategy"),
                    "complete_rule": "all_required_features_computable",
                    "missing_aware_rule": "all_required_features_have_valid_measurement_opportunity_and_explicit_qc",
                }
            )
