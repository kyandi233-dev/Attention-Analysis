...
需要特别注意：当前 `nir/` 包目录中**尚未看到以独立模块形式明确命名的 YOLO26n eye detector、tracking、RITnet segmentation 或最终 full-run orchestration**。这不代表正式 NIR 全量没有运行；它只说明“当时实际使用的最终代码入口和模块落点”仍需要在仓库中继续核验，不能用旧 `cli.py` 或旧 sequence 代码代替说明。
...
5. 下一步优先查明正式 NIR 全量运行的真实 YOLO + tracking + RITnet 代码位置，再决定是否需要调整源码模块边界。
