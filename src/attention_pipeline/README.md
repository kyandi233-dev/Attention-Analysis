...
旧通用 `cli.py` 与旧 `behavior/`/NIR benchmark-sequence 代码不再作为 current 官方入口；正式 Behavior 使用 `scripts/sart_formal_analysis.py`，正式 NIR 使用 `runtime/nir-formal/`。历史兼容源码是否最终删除需按仓库删除规则单独授权。
