...
SPECS = (
    ("behavior_rt_level", "rt_level_median", "go_correct_rt_median_ms", "RT level (median)", "ms",
     "primary", "method_frozen_researcher_median", "finite probe-level median",
     "researcher-frozen first-round RT level representation; rt_level_mean is a limited alternative only", True, False),
    ("behavior_rt_level", "rt_level_mean", "go_correct_rt_mean_ms", "RT level (mean)", "ms",
     "limited_alternative", "limited_alternative_representation", "finite probe-level mean",
     "limited alternative to the frozen rt_level_median representation; not an independent scientific dimension", True, False),
...
def build_behavior_science_output(formal_root: str|Path, science_root: str|Path, *, authoritative: bool=True,
    ...
    manifest={
        ...
        "rt_level_freeze_pending":False,
        "rt_level_frozen_representation":"go_correct_rt_median_ms",
        "rt_level_frozen_decision_basis":"Q1/Q2-blind redundancy freeze: Spearman rho = .94/.93/.93/.91 (session/block/cycle/probe); mean minus median = 9.63 ms on average and the mean is more sensitive to a few slow responses",
        "rt_level_limited_alternatives":["go_correct_rt_mean_ms"],
        "figure_n":int(len(figures)),"figure_ids":figures.get("figure_id",pd.Series(dtype=str)).astype(str).tolist(),
