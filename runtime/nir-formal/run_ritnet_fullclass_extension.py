"""Post-hoc NVIDIA/CUDA RITnet four-class extension for one completed NIR run.

The source formal pipeline already saved frame indices and exact ROI coordinates.
This extension therefore does not re-run YOLO. It re-crops the same native eye
ROIs, uses the frozen fixed-b16 FP32 RITnet ONNX model through CUDAExecutionProvider,
and retains background/sclera/iris/pupil structure plus sparse QC images.

For cross-hardware consistency, production pupil AND iris geometry are derived
from the same ONNX hard-label map. The old PyTorch-CUDA pupil columns remain in
source eyes.csv for provenance/audit but are not mixed into the primary ratio.
"""
...
from ritnet_fullclass_qc import QCSampler, build_qc_anchor_frames, save_qc_pair
from ritnet_fullclass_runtime import RitnetFullClassRuntime

PARITY_TOLERANCE = 1e-3
PUPIL_GEOMETRY_SOURCE = "onnx_labels"
...
def pupil_parity(source: dict[str, str], metrics: dict[str, Any]) -> dict[str, Any]:
    """Audit ONNX full-class pupil against the source formal PyTorch pupil."""
...
def extension_completion_valid(
    ...
    *,
    ...
    if not bool(
        ...
        and marker.get("inference_backend") == INFERENCE_BACKEND
        and marker.get("pupil_geometry_source") == PUPIL_GEOMETRY_SOURCE
    ...
    if expected_images < 0 or not qc_dir.is_dir():
        return False
    return sum(1 for _ in qc_dir.glob("*.png")) == expected_images


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Re-run frozen RITnet ONNX on CUDA and retain all four classes"
    )
    ...
    parser.add_argument(
        "--validate-pupil",
        action="store_true",
        help="Also request pupil probability and compare ONNX pupil with source PyTorch formal pupil.",
    )
    parser.add_argument("--postprocess-workers", type=int, default=4)
    parser.add_argument(
        "--allow-model-mismatch",
        action="store_true",
        help="Compatibility flag. Source PyTorch and extension ONNX hashes are recorded separately by design.",
    )
...
def main() -> int:
    ...
    if not run_dir.is_dir() or not source_completion_path.is_file() or not source_eyes.is_file():
        raise FileNotFoundError(f"Completed formal run with completion.json and eyes.csv required: {run_dir}")
    ...
    ritnet_value = config.get("models", {}).get("ritnet_onnx")
    if not ritnet_value:
        raise KeyError("config.models.ritnet_onnx is required for NVIDIA full-class extension")
    ritnet_path = resolve_package_path(ritnet_value)
    ...
    with source_eyes.open(newline="", encoding="utf-8-sig") as handle:
        ...
        if not required.issubset(set(source_fields)):
            raise ValueError(f"eyes.csv missing required columns: {sorted(required - set(source_fields))}")
        rows = list(reader)
    if not rows:
        raise RuntimeError(f"Source eyes.csv contains no eye rows: {source_eyes}")
    for row in rows:
        if normalize_subject(row.get("subject") or subject) != subject:
            raise ValueError(f"Mixed subjects in source eyes.csv: {source_eyes}")
    ...
    prototype_labels = np.zeros((runtime.input_size[1], runtime.input_size[0]), dtype=np.uint8)
    prototype = summarize_fullclass(prototype_labels, None, analysis_size)
    ...
    provenance_fields = ["fullclass_source_pupil_reused", "fullclass_pupil_validation_mode"]
    output_fields = source_fields + [
        name for name in metric_fields + parity_fields + provenance_fields if name not in source_fields
    ]
    qc_index_fields = [
        "subject", "phase", "phase_segment", "frame_idx", "video_time_ms", "unix_ms",
        "eye", "reason", "ritnet_found", "roi_clipped", "normalization_valid",
        "labels_file", "overlay_file",
    ]
    ...
    completion_base = {
        ...
        "pupil_geometry_source": PUPIL_GEOMETRY_SOURCE,
        ...
        "required_artifacts": [
            str(outputs["csv"]), str(outputs["summary"]), str(outputs["manifest"]), str(outputs["qc_index"])
        ],
    }
    ...
    decoded_frames = 0
    decode_cpu_ms = crop_cpu_ms = preprocess_cpu_ms = 0.0
    gpu_ms = postprocess_cpu_ms = csv_write_ms = qc_image_write_ms = 0.0
    processed_rows = normalization_valid_count = parity_ok_count = parity_mismatch_count = 0
    ...
    def produce_batch() -> dict[str, Any] | None:
        ...
        while len(items) < runtime.FIXED_BATCH_SIZE and current_frame <= max_frame:
            ...
            ok, frame = cap.read()
            decode_cpu_ms += (time.perf_counter() - started) * 1000.0
            ...
            if current_frame in target_set:
                for source_row in rows_by_frame[current_frame]:
                    started = time.perf_counter()
                    x1, y1, x2, y2 = (
                        parse_int(source_row["roi_x1"]), parse_int(source_row["roi_y1"]),
                        parse_int(source_row["roi_x2"]), parse_int(source_row["roi_y2"]),
                    )
                    ...
                    crop_cpu_ms += (time.perf_counter() - started) * 1000.0
        ...
        if not items:
            return None
        started = time.perf_counter()
        tensor, valid_size, _ = runtime.prepare_batch([roi for _, roi in items])
        preprocess_cpu_ms += (time.perf_counter() - started) * 1000.0
        return {"items": items, "tensor": tensor, "valid_size": valid_size}

    def postprocess_one(
        source_row: dict[str, str], labels: np.ndarray, pupil_probability: np.ndarray | None
    ) -> tuple[dict[str, Any], dict[str, Any], float]:
        started = time.perf_counter()
        metrics = summarize_fullclass(labels, pupil_probability, analysis_size)
        parity = pupil_parity(source_row, metrics) if args.validate_pupil else {
    ...
    def consume_batch(
        ...
        batch_futures: list[tuple[dict[str, str], np.ndarray, np.ndarray, Future]],
    ) -> None:
        nonlocal processed_rows, normalization_valid_count, parity_ok_count, parity_mismatch_count
        nonlocal postprocess_cpu_ms, csv_write_ms, qc_image_write_ms
        for source_row, roi_gray, labels, future in batch_futures:
            ...
            output_row["fullclass_source_pupil_reused"] = False
            output_row["fullclass_pupil_validation_mode"] = bool(args.validate_pupil)

            started = time.perf_counter()
            writer.writerow(output_row)
            csv_write_ms += (time.perf_counter() - started) * 1000.0

            reasons = qc_sampler.select(source_row, metrics)
            if reasons:
                started = time.perf_counter()
                labels_path, overlay_path = save_qc_pair(
                    outputs["qc_dir"], subject, source_row, roi_gray, labels
                )
                qc_image_write_ms += (time.perf_counter() - started) * 1000.0
                qc_records.append({
                    "subject": subject,
            ...
            for key in (
                "background_fraction", "sclera_fraction", "iris_fraction", "pupil_fraction", "ocular_fraction"
            ):
                fraction_sums[key] += float(metrics[key])

    producer_pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ritnet-prep")
    post_pool = ThreadPoolExecutor(max_workers=int(args.postprocess_workers), thread_name_prefix="ritnet-post")
    post_batches: deque[list[tuple[dict[str, str], np.ndarray, np.ndarray, Future]]] = deque()
        ...
        with outputs["csv"].open("w", newline="", encoding="utf-8-sig") as out_handle:
            ...
            while True:
                ...
                labels_batch, pupil_prob_batch, _ = runtime.infer_prepared(
                    batch["tensor"], int(batch["valid_size"]),
                    include_pupil_probability=bool(args.validate_pupil),
                )
                gpu_ms += (time.perf_counter() - started) * 1000.0

                futures: list[tuple[dict[str, str], np.ndarray, np.ndarray, Future]] = []
                for index, (source_row, roi_gray) in enumerate(batch["items"]):
                    probability = pupil_prob_batch[index] if pupil_prob_batch is not None else None
                    labels = labels_batch[index]
                    future = post_pool.submit(postprocess_one, source_row, labels, probability)
    ...
    summary = {
        ...
        "pupil_parity_ok_fraction": parity_ok_count / processed_rows if args.validate_pupil else None,
        "qc_sampling": {
        ...
        "mean_class_fractions": {
            ...
            for key in ("background_fraction", "sclera_fraction", "iris_fraction", "pupil_fraction", "ocular_fraction")
        },
    }
    ...
    manifest = {
        ...
        "qc_sampling": {
            ...
            "palette_bgr": {str(key): list(value) for key, value in QC_PALETTE_BGR.items()},
            "anomaly_reasons": ["roi_clipped", "ritnet_missing", "normalization_invalid", "ocular_fragmented"],
            "qc_dir": str(outputs["qc_dir"]),
            "qc_index": str(outputs["qc_index"]),
        },
        "time_mapping": {
            "copied_from_source_eyes_csv": True,
            "fields": ["phase", "phase_segment", "frame_idx", "video_time_ms", "unix_ms", "phase_time_ms"],
            "note": "Align behavior later by unix_ms against behavior absolute timestamps; do not rerun RITnet for trial alignment.",
        },
        "notes": [
            "Original eyes.csv is never modified.",
            "YOLO is not re-run; saved frame_idx and ROI coordinates are reused exactly.",
            "Production requests only hard labels_u8; pupil and iris geometry both come from that ONNX label map.",
            "The frozen ONNX SHA is shared with the AMD full-class extension; CUDA and DirectML are execution-provider differences only.",
            "RITnet remains 640x400, FP32, fixed batch=16; no FP16 or reduced-resolution path is used.",
            "Source PyTorch pupil columns remain available for provenance; validation mode can audit ONNX-vs-source parity.",
            "QC PNGs are sparse deterministic audit artifacts; the CSV remains the complete per-eye dataset.",
            "Ocular aperture fields are QC/eye-openness candidates, not validated blink or PERCLOS labels.",
        ],
    }
