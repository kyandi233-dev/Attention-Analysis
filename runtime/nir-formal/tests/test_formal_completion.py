...
def test_identity_mismatch_forces_rerun(tmp_path):
    run_dir, identity = _make_run(tmp_path)
    expected = {**identity, "package_version": "9.9.9"}

    result = validate_completion(run_dir, expected)
...
def test_missing_frame_fails_even_when_marker_claims_complete(tmp_path):
    ...
    # Re-publish the marker after corrupting the artifact so the failure is about
    # content, not merely marker ordering.
