"""Fixed-b16 FP32 RITnet full-class inference through ONNX Runtime CUDA.

This adapter intentionally uses the same frozen RITnet ONNX graph used by the
AMD DirectML full-class extension. The execution provider differs (CUDA vs DML),
but the model SHA, 640x400 input, FP32 precision, preprocessing, class mapping,
and 320x160 analysis coordinates remain harmonized.
"""
...
class RitnetFullClassRuntime:
    FIXED_BATCH_SIZE = RitnetOnnxRuntime.FIXED_BATCH_SIZE
    ...
    def prepare_batch(
        ...
        """CPU-only preprocessing while the previous batch can execute on CUDA."""
        if not roi_grays:
    ...
    def infer_prepared(
        ...
        *,
        ...
        """Run one fixed-b16 CUDA batch; production requests hard labels only."""
        output_names = (
    ...
    def infer_batch(
        ...
        *,
        ...
        """Compatibility wrapper for benchmarks and local validation."""
