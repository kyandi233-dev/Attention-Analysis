...
面向用户说明 producer、运行状态、QC、模型、设备差异或分析进度时，默认用中文人话先解释结论和实际影响。内部 agent 可以继续使用完整英文技术记录、路径、schema、manifest、gate、commit 和状态词，但不要把这些材料原样作为用户说明。

除非用户明确要求详细技术版，阶段性汇报优先回答：
...
专业术语第一次出现时要解释。`producer`、`QC`、`gate`、`manifest`、`schema`、`PARTIAL/BLOCKED` 等词不能只报状态；必须同时说明它对“数据能不能用、结果能不能信、正式分析能不能继续”意味着什么。

不要用长串英文文件名、路径、日志或 agent 交接文本替代解释。证据路径和版本信息应放在结论之后，需要时再展开。若结果仍不确定，要明确区分“已经观察到的事实”“当前判断”“还没有解决的问题”。
