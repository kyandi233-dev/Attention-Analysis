...
- **Project（项目看板）**：显示 Phase / Status / Owner 等总体进度；不保存正式方法定义。
...
- **Repository docs（仓库文档）**：只保存需要长期版本控制的科研计划、数据契约、已接受决策和历史工作记录；不保存滚动式临时任务/最新运行结果。
...
- 用户不需要记任务编号。当前本地任务以仓库中仍处于 open 状态、标题明确为 NIR cohort 执行任务的独立 Issue 为准。
